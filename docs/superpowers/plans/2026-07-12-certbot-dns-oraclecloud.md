...
- no production module uses `except Exception`, `except BaseException`, or a bare `except`; and
- Python 3.14 can import and test the package despite its new behavior of skipping hidden `.pth` files generated by the current uv/Hatchling editable-install path.
...
Also make the Python 3.14 verification path explicit and reproducible. Current Python 3.14 skips the hidden `_editable_impl_*.pth` file produced by the latest uv/Hatchling editable install. Use a project-owned configuration or non-editable verification path that imports the real package without ad-hoc shell `PYTHONPATH`, and cover it in CI/final verification. Do not reduce the declared Python 3.14 support.
